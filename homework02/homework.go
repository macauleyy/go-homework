...
func basicGoroutine() {
	...
	wg.Add(2)
	// 两个打印任务并发执行，使用 WaitGroup 等待它们真正结束
	go func() { defer wg.Done(); printOddNumber() }()
	go func() { defer wg.Done(); printEvenNumber() }()
	wg.Wait()
	fmt.Println()
...
1. 题目 ：定义一个 Shape 接口，包含 Area() 和 Perimeter() 两个方法然后创建 Rectangle 和 Circle 结构体，实现 Shape 接口在主函数中，创建这两个结构体的实例，并调用它们的 Area() 和 Perimeter() 方法
...
func mix(shape Shape) {
	fmt.Printf("\n面积：%.2f，周长：%.2f", shape.Area(), shape.Perimeter())
}

/*
2. 题目 ：使用组合的方式创建一个 Person 结构体，包含 Name 和 Age 字段，再创建一个 Employee 结构体，组合 Person 结构体并添加 EmployeeID 字段为 Employee 结构体实现一个 PrintInfo() 方法，输出员工的信息
  - 考察点 ：组合的使用、方法接收者
*/
type Person struct {
	Name string
...
1. 题目 ：编写一个程序，使用通道实现两个协程之间的通信一个协程生成从1到10的整数，并将这些整数发送到通道中，另一个协程从通道中接收这些整数并打印出来
  - 考察点 ：通道的基本使用、协程间通信
*/
func demo1() {
	m := make(chan int)
...
1. 题目 ：编写一个程序，使用 sync.Mutex 来保护一个共享的计数器启动10个协程，每个协程对计数器进行1000次递增操作，最后输出计数器的值
...
/*
2. 题目 ：使用原子操作（ sync/atomic 包）实现一个无锁的计数器启动10个协程，每个协程对计数器进行1000次递增操作，最后输出计数器的值
  - 考察点 ：原子操作、并发数据安全
*/
func demo4() int64 {
	var (
...
func main() {
	...
	demo1()                             //1. 题目 ：编写一个程序，使用通道实现两个协程之间的通信一个协程生成从1到10的整数，并将这些整数发送到通道中，另一个协程从通道中接收这些整数并打印出来
	demo2()                             //2. 题目 ：实现一个带有缓冲的通道，生产者协程向通道中发送100个整数，消费者协程从通道中接收这些整数并打印
	fmt.Printf("\ndemo3的值：%d", demo3()) //1. 题目 ：编写一个程序，使用 sync.Mutex 来保护一个共享的计数器启动10个协程，每个协程对计数器进行1000次递增操作，最后输出计数器的值
	fmt.Printf("\ndemo4的值：%d", demo4()) //2. 题目 ：使用原子操作（ sync/atomic 包）实现一个无锁的计数器启动10个协程，每个协程对计数器进行1000次递增操作，最后输出计数器的值
